...
def show_ownership_module():
    ...
    def is_owner_match(row, owners):
        return (row['parent'] in owners or 
                row['immediate source'] in owners or 
                row['source operator'] in owners)
    ...
    def update_owner_options():
        ...
        owner_options = (pd.concat([df_ownership[mask]['parent'], 
                                df_ownership[mask]['immediate source'], 
                                df_ownership[mask]['source operator']], ignore_index=True)
    ...
    # create table
    df_table = df_selected[['asset_id', 'asset_name', 'subsector', 'asset_type', 'parent_name', 'overall_share_percent', 'immediate_source_owner', 'source_operator', 'iso3_country', 'activity_units', 'activity', 'emissions_quantity', 'net_reduction_potential', 'ef_asset']].drop_duplicates().head(500)
